...
export const RehabAgent: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in">
      ...
      {(!refData && !lessonData) && (
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="relative">
            ...
            <input className="w-full pl-12 pr-4 py-4 text-lg border rounded-xl outline-none focus:ring-2 focus:ring-brand-500" placeholder="Ex: Hérnia de Disco..." value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} />
      ...
      {refStatus === LoadingState.LOADING && <div className="py-24 text-center"><Loader2 className="h-12 w-12 animate-spin mx-auto text-brand-500"/><p className="mt-4">Analisando caso...</p></div>}
      <AssessmentModal isOpen={isAssessmentOpen} initialQuery={query} onComplete={handleAssessmentComplete} onCancel={() => setIsAssessmentOpen(false)} initialHistory={assessmentHistory} />
    </div>
  );
};